...
import {
  ...
  isTerrainEdit,
  isTerrainRegion,
} from "../../modules/editor/objectTypes.js";
import {
  getDistrictType,
  DISTRICT_TYPES,
  ROAD_KINDS,
  ROAD_SURFACES,
} from "../../modules/world/MapDataUtils.js";
...
export function InspectorPanel({
  ...
  const fieldIdBrushKind = React.useId();
  const fieldIdTemplate = React.useId();

  const { document, selectedId, selected, groupLocked, itemLocked } =
    useStore<InspectorSelection>(
      store,
      (state) => {
        const selectedEntity = getEntityById(
          state.document,
        ...
        return {
          ...
          itemLocked:
            ...
            state.layers.itemLocked[state.selection.selectedId] === true,
        };
      },
      shallowEqual,
    );
  ...
  if (!selected || !selectedId) {
    return (
      ...
      >
        <HintCard className="text-center">
  ...
  return (
    <Panel
      ...
      badge={
        <Badge className="ml-auto" data-testid="inspector-type-badge">
          {typeLabel}
        </Badge>
      }
    >
      <div className="editor-form-stack">
        <FieldRow label="ID" htmlFor={fieldIdId}>
          <Input
            id={fieldIdId}
            type="text"
            readOnly
            value={getEntityLabel(document, selectedId)}
        ...
        {isTerrainRegion(selected) ? (
          ...
          <div className="editor-grid-two">
            <NumberInputField
              ...
              value={
                "center" in selected && Array.isArray(selected.center)
                  ? selected.center[0]
                  : "x" in selected
                    ? selected.x
                    : 0
        ...
        {isDistrict(selected) ? (
          <>
            <SelectField
              ...
              options={DISTRICT_TYPES.map((option) => ({
                value: option,
                label: option,
              }))}
        ...
        {isRoad(selected) ? (
          <>
            ...
            <SelectField
              ...
              options={ROAD_SURFACES.map((option) => ({
                value: option,
                label: option,
              }))}
        ...
        {isTerrainEdit(selected) ? (
          <>
            <FieldRow label="Brush Kind" htmlFor={fieldIdBrushKind}>
              <Input
                id={fieldIdBrushKind}
                type="text"
                readOnly
        ...
        {isAirport(selected) ? (
          <>
            <FieldRow label="Template" htmlFor={fieldIdTemplate}>
              <Input
                id={fieldIdTemplate}
                type="text"
                readOnly
                value={selected.template || "default"}
                data-testid="field-airport-template"
              />
            </FieldRow>
            <RangeNumberField
              label="Yaw (deg)"
              value={selected.yaw || 0}
              min={-180}
              max={180}
              step={1}
              disabled={locked}
              onChange={(value) => updateProperty("yaw", value)}
            />
          </>
        ) : null}
        ...
        {isAuthoredObject(selected) ? (
          <>
            ...
            <RangeNumberField
              label={
                selected.heightMode === "absolute"
                  ? "Altitude (m)"
                  : selected.heightMode === "sea-level"
                    ? "Sea Level Offset (m)"
                    : "Terrain Offset (m)"
              }
              value={selected.y || 0}
        ...
        !isAuthoredObject(selected) ? (
          <>
            <RangeNumberField
              label="Altitude (m)"
              value={(selected as EditorVantageEntity).y || 0}
              min={0}
              max={3000}
              step={10}
              disabled={locked}
            ...
            <RangeNumberField
              label="Tilt (deg)"
              value={(selected as EditorVantageEntity).tilt || 45}
              min={5}
              max={85}
              step={1}
              disabled={locked}
