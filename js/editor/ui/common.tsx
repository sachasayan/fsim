...
export function shallowEqual<T extends ComparableRecord>(
  valueA: T,
  valueB: T,
): boolean;
...
export function shallowEqual(valueA: unknown, valueB: unknown): boolean {
  ...
  for (const key of keysA) {
    if (
      ...
      !Object.is(
        (valueA as ComparableRecord)[key],
        (valueB as ComparableRecord)[key],
      )
    ) {
      return false;
    }
...
export function useStore<Selected>(
  ...
  const getSnapshot = React.useCallback(() => {
    ...
    if (
      ...
      isEqualRef.current(snapshotRef.current, nextValue)
    ) {
  ...
  return (
    <span
      className={cn(
        ...
        tone === "clean" &&
          "border-[color:var(--accent-glow)] bg-[rgba(56,189,248,0.08)] text-[color:var(--accent-strong)]",
      )}
      data-testid={testId}
    >
...
export function HintCard({
  ...
  tone = "default",
  children,
}: ClassNameProp & ChildrenProp & { tone?: Tone }) {
  return (
    <div
      className={cn(
        "editor-hint-card",
  ...
  return (
    <div className="editor-field">
      <div className="editor-field-label">
        ...
        {value !== undefined ? (
          <span className="editor-value-pill">{value}</span>
        ) : null}
      </div>
      {children}
    </div>
...
export function DockIntro({
  ...
  copy,
  className,
}: ClassNameProp & { title: React.ReactNode; copy?: React.ReactNode }) {
  return (
...
export function SectionHeading({
  ...
  return (
    <div className={cn("editor-subsection-title", className)}>{children}</div>
  );
}
...
export function SurfaceIcon({
  ...
  className,
  children,
}: ChildrenProp & ClassNameProp & { compact?: boolean }) {
  return (
    <span
      className={cn(
        compact ? "editor-icon-surface-compact" : "editor-icon-surface",
        className,
      )}
    >
      {children}
...
export function CheckboxField({
  ...
  return (
    <FieldRow label={label} htmlFor={inputId}>
  ...
  return (
    <FieldRow label={label} htmlFor={inputId}>
      <Select
        ...
        disabled={disabled}
      >
        <SelectTrigger id={inputId} data-testid={testId}>
          <SelectValue />
        </SelectTrigger>
        <SelectContent>
          {options.map((option) => (
...
export function RangeNumberField({
  ...
  return (
    <FieldRow
      label={label}
      value={formatControlValue(safeValue)}
      htmlFor={inputId}
    >
    ...
    <Tooltip>
      <TooltipTrigger>
        <Toggle
          ...
          id={id}
          className={cn(
            compact ? "editor-tool-toggle-compact" : "editor-tool-toggle",
            active && "active",
          )}
          data-testid={id}
        ...
        >
          ...
          {compact ? null : (
            <span className="flex flex-col items-center gap-1">
              <span className="text-sm font-bold uppercase tracking-[0.12em] text-[color:var(--text)]">
                {label}
              </span>
              <span className="font-mono text-[10px] uppercase tracking-[0.16em] text-[color:var(--text-dim)]">
                {shortcut}
              </span>
            </span>
          )}
