...
import type { EditorEntity, EditorStore, EditorStoreState } from '../core/types.js';
...
type TerrainLabController = {
    frameTerrainHydrology: () => void;
};

type TerrainGeneratorConfig = EditorStoreState['ui']['terrainLab']['draftConfig'];
type TerrainLabState = EditorStoreState['ui']['terrainLab'];

export function TerrainLabPanel({ store, controller }: { store: EditorStore; controller: TerrainLabController }) {
    const terrainLab = useStore<TerrainLabState>(store, (state) => state.ui.terrainLab);
    const selectedEntity = useStore<EditorEntity | null>(store, (state) => getEntityById(state.document, state.selection.selectedId));
    const config = terrainLab.draftConfig;
    const metrics = terrainLab.lastMetadata && typeof terrainLab.lastMetadata === 'object' && 'hydrology' in terrainLab.lastMetadata
        ? (terrainLab.lastMetadata as {
            hydrology?: {
                riverCount?: number;
                lakeCount?: number;
                summary?: {
        ...
        }).hydrology || { riverCount: 0, lakeCount: 0, summary: { cliffCoverage: 0, gorgeCoverage: 0, peakRelief: 0 } }
        : { riverCount: 0, lakeCount: 0, summary: { cliffCoverage: 0, gorgeCoverage: 0, peakRelief: 0 } };
    const mountainDiagnosis: string[] = [];

    if ((config.macro?.continentalAmplitude ?? 0) <= 80 && (config.macro?.ridgeAmplitude ?? 0) >= 1000) mountainDiagnosis.push('Land Lift is very low for the current Mountain Height, so peaks will read as isolated bumps instead of full mountain ranges.');
    if ((config.macro?.valleyAmplitude ?? 0) <= 10) mountainDiagnosis.push('Valley Cut is near zero, which effectively disables most large valley carving.');
    if ((config.macro?.foothillAmplitude ?? 0) < 80 && (config.macro?.ridgeAmplitude ?? 0) > 900) mountainDiagnosis.push('Mountain Spread is low, so ridges stay narrow and the terrain can feel less dramatic at a distance.');
    if ((config.landforms?.canyonDepth ?? 0) > 0.72 && (config.hydrology?.gorgeStrength ?? 0) < 0.45) mountainDiagnosis.push('Canyon Depth is high, but Gorge Strength is low, so channels may read wide without carving hard enough.');
    if ((config.macro?.plateauHeight ?? 0) > 250 && (config.macro?.escarpmentStrength ?? 0) < 0.35) mountainDiagnosis.push('Plateau Height is high while Escarpment Strength is low, so shelves may feel soft instead of cliffy.');
    if ((config.macro?.summitSharpness ?? 0) > 0.7 && (config.macro?.massifStrength ?? 0) < 0.35) mountainDiagnosis.push('Peak Sharpness is very high, but Massif Strength is low, so you may get pointy ridges without enough heroic summits.');
    ...
    function applyPreset(presetName: string) {
        const nextConfig = applyTerrainGeneratorPreset(config, presetName) as TerrainGeneratorConfig;
        ...
        Object.entries(nextConfig).forEach(([key, value]) => {
            if (key === 'macro' || key === 'hydrology' || key === 'preview' || key === 'landforms') {
        ...
        <Panel title="Terrain Lab" badge={<Badge className="ml-auto">{previewBadge}</Badge>}>
            <div className="editor-form-stack">
                ...
                <HintCard>
                    ...
                    <p>Current preset: {((TERRAIN_GENERATOR_PRESETS as Record<string, { preset?: string }>)[config.preset || 'balanced'] || (TERRAIN_GENERATOR_PRESETS as Record<string, { preset?: string }>).balanced).preset}</p>
                </HintCard>
                ...
                <SelectField
                    label="Overlay"
                    value={terrainLab.selectedOverlay || 'height'}
                    options={TERRAIN_PREVIEW_OVERLAYS.map((value: string) => ({
                        value,
                ...
                />
                <HintCard>
                    <p>Rivers: {metrics.riverCount || 0}</p>
                    <p>Lakes: {metrics.lakeCount || 0}</p>
