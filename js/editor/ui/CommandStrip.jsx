...
export function CommandStrip({ store, controller, onSave, onRebuild }) {
    const { saveState, dirty, snappingEnabled, undoCount } = useStore(store, (state) => ({
    ...
    return (
        <div className="editor-topbar-commands flex flex-wrap items-center justify-between gap-3" data-testid="command-strip">
            ...
            <div className="flex flex-wrap items-center gap-3">
                ...
                <CommandButton testId="save-button" title={saveLabel} onClick={onSave} variant="accent" className="min-w-[156px]" iconPath={saveState === 'saved' ? 'M5 12l4 4L19 7' : 'M6 4h10l4 4v12H6zM9 4v6h6V4M9 20v-6h6v6'}>
                    {saveLabel}
                </CommandButton>
            </div>
        </div>
