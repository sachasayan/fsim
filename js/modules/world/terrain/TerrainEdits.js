...
function getDistanceToSegment(x, z, ax, az, bx, bz) {
    ...
    if (lenSq <= 1e-6) { const dx = x - ax; const dz = z - az; return Math.sqrt(dx * dx + dz * dz); }
    const t = Math.max(0, Math.min(1, ((x - ax) * abx + (z - az) * abz) / lenSq));
    const px = ax + abx * t;
    const pz = az + abz * t;
    const dx = x - px; const dz = z - pz; return Math.sqrt(dx * dx + dz * dz);
}
...
function getEditInfluence(edit, x, z) {
    ...
    if (points?.length) {
        ...
        for (let i = 0; i < points.length; i++) {
            ...
            const dx = x - px; const dz = z - pz; minDist = Math.min(minDist, Math.sqrt(dx * dx + dz * dz));
            if (i > 0) {
