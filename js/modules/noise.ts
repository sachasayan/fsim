...
function noise(x, y, z) {
  ...
  // We revert manual inlining of grad as it seemed slower, we just un-nest grad
  let g000 = grad(permutation[AA], x, y, z);
  let g100 = grad(permutation[BA], x - 1, y, z);
  let g010 = grad(permutation[AB], x, y - 1, z);
  let g110 = grad(permutation[BB], x - 1, y - 1, z);

  let g001 = grad(permutation[AA + 1], x, y, z - 1);
  let g101 = grad(permutation[BA + 1], x - 1, y, z - 1);
  let g011 = grad(permutation[AB + 1], x, y - 1, z - 1);
  let g111 = grad(permutation[BB + 1], x - 1, y - 1, z - 1);

  let x00 = g000 + u * (g100 - g000);
  let x10 = g010 + u * (g110 - g010);
  let x01 = g001 + u * (g101 - g001);
  let x11 = g011 + u * (g111 - g011);
