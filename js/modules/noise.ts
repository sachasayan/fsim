...
export const Noise = {
  ...
  noise(x, y, z) {
    ...
    // Direct array access from module scope
    ...
    // Use Noise.* to avoid 'this' context bindings and access exported functions
    return Noise.lerp(
      w,
      Noise.lerp(
        v,
        Noise.lerp(u, Noise.grad(PERMUTATION[AA], x, y, z), Noise.grad(PERMUTATION[BA], x - 1, y, z)),
        Noise.lerp(u, Noise.grad(PERMUTATION[AB], x, y - 1, z), Noise.grad(PERMUTATION[BB], x - 1, y - 1, z))
      ),
      Noise.lerp(
        v,
        Noise.lerp(u, Noise.grad(PERMUTATION[AA + 1], x, y, z - 1), Noise.grad(PERMUTATION[BA + 1], x - 1, y, z - 1)),
        Noise.lerp(
          u,
          Noise.grad(PERMUTATION[AB + 1], x, y - 1, z - 1),
          Noise.grad(PERMUTATION[BB + 1], x - 1, y - 1, z - 1)
        )
      )
    );
  },
