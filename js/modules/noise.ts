...
export const Noise = {
  ...
  noise(x, y, z) {
    ...
    const gBA = grad(P[BA], x1, y, z);
    const gAB = grad(P[AB], x, y1, z);
    const gBB = grad(P[BB], x1, y1, z);

    const gAA1 = grad(P[AA + 1], x, y, z1);
    const gBA1 = grad(P[BA + 1], x1, y, z1);
    const gAB1 = grad(P[AB + 1], x, y1, z1);
    const gBB1 = grad(P[BB + 1], x1, y1, z1);

    const l1 = gAA + u * (gBA - gAA);
    const l2 = gAB + u * (gBB - gAB);
    const l3 = gAA1 + u * (gBA1 - gAA1);
    const l4 = gAB1 + u * (gBB1 - gAB1);
