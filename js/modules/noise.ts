...
export const Noise = {
  ...
  noise(x, y, z) {
    ...
    const l0 = grad(perm[AA], x, y, z);
    const l1 = grad(perm[BA], x - 1, y, z);
    const v0 = l0 + u * (l1 - l0);

    const l2 = grad(perm[AB], x, y - 1, z);
    const l3 = grad(perm[BB], x - 1, y - 1, z);
    const v1 = l2 + u * (l3 - l2);

    const f0 = v0 + v * (v1 - v0);

    const l4 = grad(perm[AA + 1], x, y, z - 1);
    const l5 = grad(perm[BA + 1], x - 1, y, z - 1);
    const v2 = l4 + u * (l5 - l4);

    const l6 = grad(perm[AB + 1], x, y - 1, z - 1);
    const l7 = grad(perm[BB + 1], x - 1, y - 1, z - 1);
    const v3 = l6 + u * (l7 - l6);
